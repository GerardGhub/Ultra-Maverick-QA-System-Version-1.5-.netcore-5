  ...
  public partial class ApplicationDbContext : IdentityDbContext<ApplicationUser>
  {
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
      ...
      modelBuilder.Entity<TaskStatusModel>().HasData(
          new TaskStatusModel() { TaskStatusID = 1, TaskStatusName = "Holding" }, //Tasks that need to be documented still
          new TaskStatusModel() { TaskStatusID = 2, TaskStatusName = "Prioritized" }, //Tasks that are placed in priority order; so need to start ASAP
          new TaskStatusModel() { TaskStatusID = 3, TaskStatusName = "Started" }, //Tasks that are currently working
          new TaskStatusModel() { TaskStatusID = 4, TaskStatusName = "Finished" }, //Tasks that are finished workng
          new TaskStatusModel() { TaskStatusID = 5, TaskStatusName = "Reverted" } //Tasks that are reverted back, with comments or issues
       );
    }
